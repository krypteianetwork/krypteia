...
*Setup instructions for a Gitian build of krypteia Core using a Debian VM or physical system.*
...
- [Building krypteia Core](#building-bitcoin-core)
...
in the krypteia repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building krypteia Core
----------------

To build krypteia Core (for Linux, OS X and Windows) just follow the steps under 'perform
...
For example:
```bash
URL=https://github.com/snguyen-vn/krypteia.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit krypteia=${COMMIT} --url bitcoin=${URL} ../bitcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit krypteia=${COMMIT} --url bitcoin=${URL} ../bitcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit krypteia=${COMMIT} --url bitcoin=${URL} ../bitcoin/contrib/gitian-descriptors/gitian-osx.yml
```
